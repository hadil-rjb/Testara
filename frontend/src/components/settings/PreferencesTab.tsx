...
export default function PreferencesTab({ onToast }: PreferencesTabProps) {
  ...
  const handleThemeChange = async (mode: 'light' | 'dark') => {
    ...
    try {
      ...
      /* theme is already applied locally, silent fail on sync */
    } finally {
      setSavingTheme(null);
    }
  };
...
function ThemeCard({ mode, label, active, loading, onClick }: ThemeCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="absolute top-3 left-3 w-10 h-1.5 rounded-full"
          style={{ backgroundColor: isLight ? '#1A1A2E' : '#F1F1F6' }}
        />
        <div
          className="absolute top-6 left-3 w-16 h-1 rounded-full opacity-60"
          style={{ backgroundColor: isLight ? '#6B7280' : '#9CA3AF' }}
        />
        <div
          className="absolute bottom-3 left-3 w-5 h-5 rounded-full"
          style={{ backgroundColor: '#654CDE' }}
        />
        <div
          className="absolute bottom-3 right-3 w-4 h-4 rounded-full flex items-center justify-center"
          style={{
            backgroundColor: isLight ? '#F1F3F9' : '#242440',
          }}
        >
          {isLight ? (
            <Sun size={10} className="text-[#1A1A2E]" />
          ) : (
            <Moon size={10} className="text-[#F1F1F6]" />
          )}
        </div>
      </div>
